...
function printHelp() {
  ...
  echo "    -c <channel name> - channel name to use (defaults to \"tolbachannel\")"
  ...
  echo
  echo "	byfn.sh generate -c tolbachannel"
  echo "	byfn.sh up -c tolbachannel -s couchdb"
  echo "        byfn.sh up -c tolbachannel -s couchdb -i 1.4.0"
  echo "	byfn.sh up -l node"
  echo "	byfn.sh down -c tolbachannel"
  echo "        byfn.sh upgrade -c tolbachannel"
  echo
...
function upgradeNetwork() {
  if [[ "$IMAGETAG" == *"1.4"* ]] || [[ $IMAGETAG == "latest" ]]; then
    docker inspect -f '{{.Config.Volumes}}' orderer.tolba.com | grep -q '/var/hyperledger/production/orderer'
    if [ $? -ne 0 ]; then
    ...
    docker-compose $COMPOSE_FILES stop orderer.tolba.com
    docker cp -a orderer.tolba.com:/var/hyperledger/production/orderer $LEDGERS_BACKUP/orderer.tolba.com
    docker-compose $COMPOSE_FILES up -d --no-deps orderer.tolba.com

    for PEER in peer0.cib.tolba.com peer1.cib.tolba.com peer0.hsbc.tolba.com peer1.hsbc.tolba.com; do
...
function networkDown() {
  ...
  if [ "$MODE" != "restart" ]; then
    ...
    rm -rf channel-artifacts/*.block channel-artifacts/*.tx crypto-config ./qnb-artifacts/crypto-config/ channel-artifacts/qnb.json
    # remove the docker-compose yaml file that was customized to the tolba
...
function replacePrivateKey() {
  ...
  CURRENT_DIR=$PWD
  cd crypto-config/peerOrganizations/cib.tolba.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
  sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
  cd crypto-config/peerOrganizations/hsbc.tolba.com/ca/
  PRIV_KEY=$(ls *_sk)
  cd "$CURRENT_DIR"
...
# and two Peer Orgs (``cib`` & ``Org2``) each managing and maintaining two peer nodes.
...
# (``peer0.cib.tolba.com`` & ``peer0.hsbc.tolba.com``).  Secondly, we point to
...
function generateChannelArtifacts() {
  ...
  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for CIBMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate ./channel-artifacts/CIBMSPanchors.tx -channelID $CHANNEL_NAME -asOrg CIBMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for CIBMSP..."
    exit 1
  fi

  echo
  echo "#################################################################"
  echo "#######    Generating anchor peer update for HSBCMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/HSBCMSPanchors.tx -channelID $CHANNEL_NAME -asOrg HSBCMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for HSBCMSP..."
    exit 1
  fi
  echo
 ...
 echo "#################################################################"
  echo "#######    Generating anchor peer update for QNBMSP   ##########"
  echo "#################################################################"
  set -x
  configtxgen -profile ThreeOrgsChannel -outputAnchorPeersUpdate \
    ./channel-artifacts/QNBMSPanchors.tx -channelID $CHANNEL_NAME -asOrg QNBMSP
  res=$?
  set +x
  if [ $res -ne 0 ]; then
    echo "Failed to generate anchor peer update for QNBMSP..."
    exit 1
  fi
  echo
}
...
CLI_DELAY=3
# channel name defaults to "tolbachannel"
