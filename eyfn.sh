...
function printHelp () {
  ...
  echo
  echo "	eyfn.sh generate -c tolbachannel"
  echo "	eyfn.sh up -c tolbachannel -s couchdb"
  echo "	eyfn.sh up -l node"
  echo "	eyfn.sh down -c tolbachannel"
  echo
...
function networkUp () {
  ...
  if [ ! -d "qnb-artifacts/crypto-config" ]; then
    generateCerts
  ...
  echo "###############################################################"
  docker exec Org3cli ./scripts/step2qnb.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
  ...
  echo "###############################################################"
  docker exec cli ./scripts/step3qnb.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
    echo "ERROR !!!! Unable to add Org3 peers on network"
    exit 1
  fi
  # finish by running the test
  docker exec Org3cli ./scripts/testqnb.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function createConfigTx () {
  ...
  echo "###############################################################"
  docker exec cli scripts/step1qnb.sh $CHANNEL_NAME $CLI_DELAY $LANGUAGE $CLI_TIMEOUT $VERBOSE
  if [ $? -ne 0 ]; then
...
function generateCerts (){
  ...
  (cd qnb-artifacts
   set -x
   cryptogen generate --config=./qnb-crypto.yaml
   res=$?
   set +x
   if [ $res -ne 0 ]; then
...
function generateChannelArtifacts() {
  ...
  (cd qnb-artifacts
   export FABRIC_CFG_PATH=$PWD
   set -x
   configtxgen -printOrg QNBMSP > ../channel-artifacts/qnb.json
   res=$?
   set +x
   if [ $res -ne 0 ]; then
  ...
  cp -r crypto-config/ordererOrganizations qnb-artifacts/crypto-config/
  echo
}
...
#
COMPOSE_FILE_COUCH_ORG3=docker-compose-couch-qnb.yaml
